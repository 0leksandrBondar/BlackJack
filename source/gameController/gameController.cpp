	...
	, _cardStack(_newCardWidget->initCardStack())
{
	...
	connect(_scene.get(), &Scene::clickOnAddCardWidget, this, &GameController::onClickedNewCardWidget);
	initGameTable();
}
...
void GameController::resetPot()
{
	_pot = 0;
}

int GameController::playerBalance() const
{
...
{
	const bool isPlayerWinner{(playerScore > dealerScore && playerScore <= 21) || (playerScore <= 21 && dealerScore > 21)};
	const bool isDealerWinner{(dealerScore > playerScore && dealerScore <= 21) || (playerScore > 21 && dealerScore <= 21)};
