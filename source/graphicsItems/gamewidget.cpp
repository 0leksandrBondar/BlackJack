...
const QString potlabel = QStringLiteral("<span style='color: green'>Pot:</span> <span style='color: yellow'>$%1</span>");
const QString balancelabel = QStringLiteral("<span style='color: green'>Balance:</span> <span style='color: yellow'>$%1</span>");
	...
	, _settingWindow{new SettingWindow(this)}
{
	...
	connect(_settingsButton.get(), &QPushButton::clicked, _settingWindow, &SettingWindow::showSettingsWindow);
	connect(_settingWindow, &SettingWindow::changeTheme, this, &GameWidget::requestChangeTheme);
}
...
void GameWidget::resetVictoryLabel()
{
	_victoryLabel->setText(DefaultText::hint);
}
